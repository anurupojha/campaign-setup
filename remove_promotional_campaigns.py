#!/usr/bin/env python3
"""
Remove 10 specified campaigns from STREAK_JOURNEY_JOB_CONFIG
"""
...
# Campaigns to remove
CAMPAIGNS_TO_REMOVE = [
    "upi_etu_rupay_act_react_assured_50_on_next",
    "snp_nonrupay_atc_streak_assignment",
    "snp_nonrupay_m1_jun_streak_assignment",
    "snp_rup_may_streak_assignment",
    "snp_swiggy_gc",
    "axis_rupay_10_april_streak_assignment",
    "axis_rupay_20_april_streak_assignment",
    "sbi_rupay_upi_act",
    "upi_reactivation_rupay_flow",
    "upi_activation_rupay_flow"
]
...
def remove_campaigns(value_obj: Dict[str, Any], campaigns: List[str]) -> Tuple[Dict[str, Any], Dict[str, int]]:
    ...
    Returns: (modified_value_obj, removal_counts)
    """
    counts = {
        'supported_ids': 0,
        'batch_rules': 0,
    ...
    # 1. Remove from supported_campaign_ids
    # First, we need to find the UUIDs for these campaign names from journey rules
    journey_rules = value_obj.get('journey_rules', {}).get('configs', [])

    # Map campaign names to UUIDs
    campaign_uuids = set()
...
def main():
    print("\n" + "="*80)
    print("REMOVE 10 OLD PROMOTIONAL CAMPAIGNS")
    print("="*80)
    ...
    config_data['updated_by'] = "campaign_cleanup_promotional"
